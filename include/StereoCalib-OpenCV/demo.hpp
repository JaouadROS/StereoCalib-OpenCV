...
class Demo {
  ...
  // Serial m_serial; // should #include "Serial.h"
...
public:
  ...
  // extractTags using m_tagDetector->extractTags(grayL);
  std::vector<AprilTags::TagDetection> extractTags(cv::Mat& image_gray) {
    return m_tagDetector->extractTags(image_gray);
  }
  ...
  void setup() {
    ...
    if (m_arduino) {
      //m_serial.open("/dev/ttyACM0"); should #include "Serial.h"
    }
  }
  ...
  void processImage(cv::Mat& image, cv::Mat& image_gray) {
    ...
    // optionally send tag information to serial port (e.g. to Arduino) // should #include "Serial.h"
    /*if (m_arduino) {
